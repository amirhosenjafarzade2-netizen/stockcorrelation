"""
Enhanced Financial Grapher with SEC Edgar Integration
...
try:
    ...
    st.warning("⚠️ Install `edgartools` for 10+ years of data: `pip install edgartools`")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                st.warning(f"Failed to fetch price data: {str(e)}")
                return pd.Series(dtype=float)
            time.sleep(1)
...
def fetch_yfinance_fundamentals(ticker: str, frequency: str):
    ...
    try:
        ...
        st.warning(f"Error fetching yfinance data: {str(e)}")
...
def render_grapher() -> None:
    st.title("📈 Enhanced Financial Grapher")
    ...
    if st.button("🚀 Load & Analyze", type="primary", use_container_width=True):
        with st.spinner(f"Fetching data for {ticker}..."):
            ...
            with col1:
                if not prices.empty:
                    st.success(f"✅ **Price**: {len(prices)} days ({prices.index.min().date()} to {prices.index.max().date()})")
            
            with col2:
                if not income.empty:
                    periods = len(income.columns)
                    date_range = f"{income.columns.min().date()} to {income.columns.max().date()}" if hasattr(income.columns, 'min') else "multiple periods"
                    source = "SEC Edgar" if use_edgar else "Yahoo Finance"
                    st.success(f"✅ **Fundamentals ({source})**: {periods} periods ({date_range})")
            ...
            if use_edgar:
                ...
                st.info("💡 **Tip**: Install `edgartools` to access 10+ years of historical data!")
