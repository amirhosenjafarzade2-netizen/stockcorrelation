...
import plotly.graph_objects as gofrom datetime 
import datetime
...
DEFAULT_MOS = 25.0TRADING_DAYS_PER_YEAR = 252

── Finviz Data Scraper (fallback / enhancement) ────────────────────────────

@st.cache_data(ttl=3600)def get_finviz_data(ticker: str) -> dict:"""Scrape key fundamentals from Finviz as fallback or supplement to yfinance."""data = {}headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'}url = f"https://finviz.com/quote.ashx?t={ticker}"try:response = requests.get(url, headers=headers, timeout=12)response.raise_for_status()soup = BeautifulSoup(response.text, 'html.parser')table = soup.find('table', {'class': 'snapshot-table2'})if table:rows = table.find_all('tr')for row in rows:cells = row.find_all('td')for i in range(0, len(cells), 2):if i + 1 < len(cells):key = cells[i].text.strip()value = cells[i+1].text.strip()try:clean_val = value.replace('%', '').replace(',', '').replace('$', '')data[key] = float(clean_val) if clean_val.replace('.', '', 1).lstrip('-').isdigit() else valueexcept:data[key] = valuereturn dataexcept Exception:return {}

@st.cache_data(ttl=3600)def get_finviz_sector_tickers(sector: str) -> list:"""Fetch all tickers from a Finviz sector with caching"""sector_map = {"Technology": "sec_technology","Healthcare": "sec_healthcare","Financials": "sec_financial","Energy": "sec_energy","Consumer Discretionary": "sec_consumercyclical","Consumer Staples": "sec_consumernoncyclical","Industrials": "sec_industrials","Basic Materials": "sec_basicmaterials","Communication Services": "sec_communicationservices","Utilities": "sec_utilities","Real Estate": "sec_realestate"}
...
@st.cache_data(ttl=3600)def fetch_stock_data(ticker: str) -> dict:"""Fetch stock fundamentals — primarily from yfinance, enhanced/fallback with Finviz."""try:ticker_clean = ticker.replace('.', '-')stock = yf.Ticker(ticker_clean)info = stock.info or {}
...
def core_valuation(inputs):"""Original Excel-style core valuation"""eps = inputs['forward_eps']pe = inputs['historical_pe']growth = inputs['analyst_growth'] / 100years = inputs['years_high_growth']desired_return = inputs['desired_return'] / 100
...
def lynch_method(inputs):"""Peter Lynch's PEG-based valuation"""eps = inputs['current_eps']growth = inputs['analyst_growth']dividend_yield = (inputs['dividend_per_share'] / inputs['current_price']) * 100 if inputs['current_price'] > 0 else 0
...
def ddm_valuation(inputs):"""Dividend Discount Model (Gordon Growth)"""dividend = inputs['dividend_per_share']growth = inputs['dividend_growth'] / 100required_return = inputs['desired_return'] / 100
...
def two_stage_dcf(inputs):"""Two-Stage DCF Model"""fcf = inputs['fcf']high_growth = inputs['analyst_growth'] / 100stable_growth = inputs['stable_growth'] / 100wacc = inputs['wacc'] / 100years = inputs['years_high_growth']shares = inputs['shares_outstanding']
...
def residual_income(inputs):"""Residual Income Model"""book_value = inputs['book_value']roe = inputs['roe'] / 100cost_of_equity = inputs['desired_return'] / 100growth = inputs['analyst_growth'] / 100years = 10
...
def reverse_dcf(inputs):"""Reverse DCF - implied growth rate"""current_price = inputs['current_price']fcf = inputs['fcf']wacc = inputs['wacc'] / 100shares = inputs['shares_outstanding']stable_growth = inputs['stable_growth'] / 100
...
def graham_intrinsic_value(inputs):"""Benjamin Graham's Formula"""eps = inputs['current_eps']growth = inputs['analyst_growth']
...
def comparable_company_analysis(inputs):"""Comparable Companies (P/E multiple)"""eps = inputs['current_eps']pe = inputs['historical_pe']intrinsic = eps * pereturn {'intrinsic_value': max(intrinsic, 0)}

def precedent_transactions(inputs):"""Precedent Transactions (with control premium)"""comps_val = comparable_company_analysis(inputs)['intrinsic_value']premium = 25.0return {'intrinsic_value': comps_val * (1 + premium / 100)}

def asset_based_valuation(inputs):"""Net Asset Value"""book_value = inputs['book_value']nav = book_value * 0.7return {'intrinsic_value': max(nav, 0)}

def sum_of_the_parts(inputs):"""Sum-of-the-Parts Valuation"""segment1 = two_stage_dcf(inputs)['intrinsic_value'] * 0.6segment2 = two_stage_dcf(inputs)['intrinsic_value'] * 0.4net_debt = inputs.get('net_debt', 0)return {'intrinsic_value': max(segment1 + segment2 - net_debt, 0)}

def eva_based_valuation(inputs):"""Economic Value Added"""eva = inputs['nopat'] - (inputs['invested_capital'] * inputs['wacc'] / 100)if eva > 0 and inputs['wacc'] > 0:mva = eva / (inputs['wacc'] / 100)return {'intrinsic_value': inputs['book_value'] + mva}return {'intrinsic_value': inputs['book_value']}

def adjusted_present_value(inputs):"""Adjusted Present Value"""unlevered = two_stage_dcf(inputs)['intrinsic_value']tax_shield = inputs['tax_rate']/100 * inputs.get('debt', 0) * 0.05pv_shield = tax_shield / 0.05 if 0.05 > 0 else 0return {'intrinsic_value': unlevered + pv_shield}

def owner_earnings(inputs):"""Owner Earnings (Buffett-style)"""oe = inputs['fcf'] if inputs['fcf'] > 0 else inputs['current_eps'] * inputs['shares_outstanding']g = inputs['analyst_growth'] / 100r = inputs['desired_return'] / 100
...
def calculate_valuation(inputs):"""Calculate intrinsic value using selected model"""model = inputs['model']current_price = inputs['current_price']mos = inputs.get('core_mos', DEFAULT_MOS)
...
def validate_inputs(inputs):"""Validate input parameters"""if inputs['current_price'] <= 0:return Falseif inputs['current_eps'] == 0 and inputs['fcf'] <= 0:return Falseif inputs['desired_return'] <= 0:return Falsereturn True

── S&P 500 List ────────────────────────────────────────────────────────────

@st.cache_data(ttl=86400)def get_sp500_tickers():"""Fetch S&P 500 ticker list"""url = "https://en.wikipedia.org/wiki/List_of_S%26P_500_companies"try:df = pd.read_html(url)[0]df = df[['Symbol', 'Security', 'GICS Sector']]df['Symbol'] = df['Symbol'].str.replace('.', '-', regex=False)return dfexcept:return pd.DataFrame({'Symbol': ['AAPL','MSFT','GOOGL','AMZN','META'],'Security': ['Apple','Microsoft','Alphabet','Amazon','Meta'],'GICS Sector': ['Information Technology']*5})

── Screener Functions ──────────────────────────────────────────────────────

def run_sp500_screener(model, min_undervaluation=10.0, selected_sectors=None, max_stocks=100):"""Screen S&P 500 for undervalued stocks"""sp500 = get_sp500_tickers()if selected_sectors:sp500 = sp500[sp500['GICS Sector'].isin(selected_sectors)]
...
def run_sector_screener(sector, model, min_undervaluation=10.0, max_stocks=50):"""Screen a specific sector from Finviz"""status = st.empty()status.text(f"Fetching {sector} tickers from Finviz...")
...
def sensitivity_analysis(inputs, param1='analyst_growth', param2='wacc', steps=10):"""Run sensitivity analysis on two parameters"""base_val = calculate_valuation(inputs)['intrinsic_value']
...
def render_advanced_valuation():st.subheader("💹 Advanced Valuation & Screener")st.caption("Multi-model intrinsic value calculator with S&P 500 and sector-based undervalued stock screeners")
...
if name == "main":st.set_page_config(page_title="Advanced Valuation & Screener", layout="wide")render_advanced_valuation()
